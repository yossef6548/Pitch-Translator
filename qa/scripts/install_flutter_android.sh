...
if [[ ! -f "$TMP_ZIP" ]]; then
  curl -L -o "$TMP_ZIP" https://dl.google.com/android/repository/commandlinetools-linux-13114758_latest.zip
fi
...
if ! avdmanager list avd | rg -q "$AVD_NAME"; then
  echo no | avdmanager create avd -n "$AVD_NAME" -k "system-images;android-35;google_apis;x86_64" -d pixel_6
fi
